...
struct OnboardingView: View {
    ...
    private var progressBar: some View {
        GeometryReader { geometry in
            ZStack(alignment: .leading) {
                Rectangle()
                    .fill(Color.back2.opacity(0.3))
                
                Rectangle()
                    .fill(
                        LinearGradient(
                            colors: [.back2, .back2, .back1],
                            startPoint: .leading,
                            endPoint: .trailing
                        )
                    )
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 4)
                    .stroke(Color.text2, lineWidth: 1)
            )
        }
        .frame(height: 10)
        .padding(.horizontal)
    ...
    private func pageView(for page: OnboardingPage) -> some View {
        VStack(spacing: 30) {
            Text(page.title)
                .font(.customFont(font: .lato, style: .light, size: .h1))
            ...
            GIFViewStatication(gifName: page.gifName)
                .clipShape(RoundedRectangle(cornerRadius: 20))
                .background {
                    RoundedRectangle(cornerRadius: 20)
                        .fill(Color.black)
                        .shadow(color: Color.text2.opacity(0.6), radius: 15, x: 0, y: 6)
                }
                .overlay {
                    RoundedRectangle(cornerRadius: 20)
                        .strokeBorder(
            ...
            Text(page.description)
                .font(.customFont(font: .lato, style: .light, size: .h2))
